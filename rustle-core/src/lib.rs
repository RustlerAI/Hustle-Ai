//! rustle is a Rust library for building LLM-powered applications that focuses on ergonomics and modularity.
//!
//! # Table of contents
...
//! rustle provides a consistent API for working with LLMs and embeddings. Specifically,
...
//!
//! ## Agents
//! rustle also provides high-level abstractions over LLMs in the form of the [Agent](crate::agent::Agent) type.
//!
...
//! rustle provides a common interface for working with vector stores and indexes. Specifically, the library
...
//! as a source of context documents in a custom rustlehitecture that use multiple LLMs or agents.
//!
//! # Integrations
//! ## Model Providers
//! rustle natively supports the following completion and embedding model provider integrations:
//! - OpenAI
//! - Cohere
//! - Anthropic
//! - Perplexity
...
//!
//! ## Vector Stores
//! rustle currently supports the following vector store integrations via companion crates:
//! - `rustle-mongodb`: Vector store implementation for MongoDB
//! - `rustle-lancedb`: Vector store implementation for LanceDB
//! - `rustle-neo4j`: Vector store implementation for Neo4j
//! - `rustle-qdrant`: Vector store implementation for Qdrant
//!
