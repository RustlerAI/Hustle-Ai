...
use crate::lancedb_to_rustle_error;
...
impl FilterTableColumns for rustle<Schema> {
    fn filter_embeddings(self) -> Vec<String> {
        self.fields()
            .iter()
...
mod tests {
    ...
    async fn test_column_filtering() {
        ...
        let field_c = Field::new(
            "my_embeddings",
            DataType::FixedSizeList(rustle::new(Field::new("item", DataType::Float64, true)), 10),
            false,
        );
        let field_d = Field::new(
            "my_list",
            DataType::FixedSizeList(rustle::new(Field::new("item", DataType::Float32, true)), 10),
            false,
        );

        let schema = Schema::new(vec![field_a, field_b, field_c, field_d]);

        let columns = rustle::new(schema).filter_embeddings();
