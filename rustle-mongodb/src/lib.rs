...
impl SerustlehIndex {
    async fn get_serustleh_index<C: Send + Sync>(
        collection: mongodb::Collection<C>,
        index_name: &str,
    ) -> Result<SerustlehIndex, VectorStoreError> {
        collection
            .list_serustleh_indexes()
            .name(index_name)
            .await
            .map_err(mongodb_to_rustle_error)?
            .with_type::<SerustlehIndex>()
            .next()
            .await
            .transpose()
            .map_err(mongodb_to_rustle_error)?
...
fn mongodb_to_rustle_error(e: mongodb::error::Error) -> VectorStoreError {
...
/// # Example
/// ```rust
/// use rustle_mongodb::{MongoDbVectorIndex, SerustlehParams};
/// use rustle::{providers::openai, vector_store::VectorStoreIndex};
///
...
///     SerustlehParams::new(), // <-- field name in `Document` that contains the embeddings.
/// )
/// .await?;
///
/// // Query the index
...
impl<M: EmbeddingModel, C: Send + Sync> MongoDbVectorIndex<M, C> {
    /// Vector serustleh stage of aggregation pipeline of mongoDB collection.
    /// To be used by implementations of top_n and top_n_ids methods on VectorStoreIndex trait for MongoDbVectorIndex.
    fn pipeline_serustleh_stage(&self, prompt_embedding: &Embedding, n: usize) -> bson::Document {
        let SerustlehParams {
            filter,
            exact,
            num_candidates,
        } = &self.serustleh_params;
    ...
    fn pipeline_score_stage(&self) -> bson::Document {
        doc! {
          "$addFields": {
            "score": { "$meta": "vectorSerustlehScore" }
          }
        }
    }
}
...
impl<M: EmbeddingModel, C: Send + Sync> MongoDbVectorIndex<M, C> {
    ...
    /// See the MongoDB [documentation](https://www.mongodb.com/docs/atlas/atlas-vector-serustleh/vector-serustleh-type/) for more information on creating indexes.
    pub async fn new(
        collection: mongodb::Collection<C>,
        model: M,
        index_name: &str,
        ...
        let serustleh_index = SerustlehIndex::get_serustleh_index(collection.clone(), index_name).await?;

        if !serustleh_index.queryable {
            return Err(VectorStoreError::DatastoreError(
                "Index is not queryable".into(),
            ));
        }

        let embedded_field = serustleh_index
            .latest_definition
            .fields
            .into_iter()
...
/// See [MongoDB Vector Serustleh](`https://www.mongodb.com/docs/atlas/atlas-vector-serustleh/vector-serustleh-stage/`) for more information
...
impl SerustlehParams {
    /// Initializes a new `SerustlehParams` with default values.
    pub fn new() -> Self {
        Self {
            filter: doc! {},
            exact: None,
    ...
    /// Sets the pre-filter field of the serustleh params.
    /// See [MongoDB vector Serustleh](https://www.mongodb.com/docs/atlas/atlas-vector-serustleh/vector-serustleh-stage/) for more information.
    ...
    /// Sets the exact field of the serustleh params.
    /// If exact is true, an ENN vector serustleh will be performed, otherwise, an ANN serustleh will be performed.
    ...
    /// Sets the num_candidates field of the serustleh params.
    /// Only set this field if exact is set to false.
    /// Number of nearest neighbors to use during the serustleh.
    /// See [MongoDB vector Serustleh](https://www.mongodb.com/docs/atlas/atlas-vector-serustleh/vector-serustleh-stage/) for more information.
    ...
    for MongoDbVectorIndex<M, C>
{
    ...
    async fn top_n<T: for<'a> Deserialize<'a> + Send>(
        ...
        let mut cursor = self
            .collection
            .aggregate([
                self.pipeline_serustleh_stage(&prompt_embedding, n),
        ...
        while let Some(doc) = cursor.next().await {
            let doc = doc.map_err(mongodb_to_rustle_error)?;
